import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

Color primaryColor = Color(0xff70BACB);
Color blueColor = Color(0xff47C4E0);
Color blue2Color = Color(0xff0582CA);
Color greyColor = Color(0xffB8B8B8);
Color grey2Color = Color(0xff828282);
Color whiteColor = Color(0xffffffff);
Color yellowColor = Color(0xffFDBF00);
Color blackColor = Color(0xffF000000);

TextStyle primaryTextStyle = GoogleFonts.poppins()
    .copyWith(color: primaryColor, fontWeight: FontWeight.w500);

TextStyle blueTextStyle = GoogleFonts.poppins()
    .copyWith(color: blueColor, fontWeight: FontWeight.w500);

TextStyle blue2TextStyle = GoogleFonts.poppins()
    .copyWith(color: blue2Color, fontWeight: FontWeight.w500);

TextStyle greyTextStyle = GoogleFonts.poppins()
    .copyWith(color: greyColor, fontWeight: FontWeight.w500);

TextStyle grey2TextStyle = GoogleFonts.poppins()
    .copyWith(color: grey2Color, fontWeight: FontWeight.w500);

TextStyle whiteTextStyle = GoogleFonts.poppins()
    .copyWith(color: whiteColor, fontWeight: FontWeight.w500);

TextStyle yellowTextStyle = GoogleFonts.poppins()
    .copyWith(color: yellowColor, fontWeight: FontWeight.w500);

TextStyle blackTextStyle = GoogleFonts.poppins()
    .copyWith(color: blackColor, fontWeight: FontWeight.w500);
